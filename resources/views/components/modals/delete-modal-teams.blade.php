...
<script>
    ...
    function confirmDeleteTeams() {
        const selectedTeams = Array.from(document.querySelectorAll('input[name="selected_teams[]"]:checked')).map(checkbox => checkbox.value);
        if (selectedTeams.length > 0) {
            sendPostRequest('{{ route('teams.destroySelected') }}', selectedTeams);
        } else {
