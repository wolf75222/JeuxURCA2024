<!-- Show event modal pour l'utilisateur {{ $event->id }} -->
<div id="showEventModal-{{ $event->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow p-5">
            ...
            <div class="flex justify-between items-start pb-4 mb-4 border-b dark:border-gray-600">
                ...
                <button type="button"
                    ...
                    data-modal-close="showEventModal-{{ $event->id }}"
                    onclick="closeModal('showEventModal-{{ $event->id }}')">
            ...
            <div class="text-gray-800 dark:text-white">
                <div class="mb-4"><strong>ID :</strong> {{ $event->id }}</div>
                <div class="mb-4"><strong>Nom :</strong> {{ $event->name }}</div>


                <div class="mb-4"><strong>Créé le :</strong> {{ $event->created_at }}</div>
                <div class="mb-4"><strong>Mis à jour le :</strong> {{ $event->updated_at }}</div>
            </div>
        </div>
    </div>
</div>
