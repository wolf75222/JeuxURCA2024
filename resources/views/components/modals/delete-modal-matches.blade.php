...
@props(['modalId' => 'deletematchesModal', 'message' => 'Êtes-vous sûr de vouloir supprimer ce matche ?'])
...
<script>
    ...
    function confirmDeletematches() {
        const selectedmatches = Array.from(document.querySelectorAll('input[name="selected_matches[]"]:checked')).map(checkbox => checkbox.value);
        if (selectedmatches.length > 0) {
            sendPostRequest('{{ route('matches.destroySelectedmatches') }}', selectedmatches);
        } else {
