...
@props(['modalId' => 'deleteTeamModal', 'message' => 'Êtes-vous sûr de vouloir supprimer ce groupe ?'])
...
<script>
    ...
    function confirmDeletegroups() {
        const selectedgroups = Array.from(document.querySelectorAll('input[name="selected_groups[]"]:checked')).map(checkbox => checkbox.value);
        if (selectedgroups.length > 0) {
            sendPostRequest('{{ route('groups.destroySelected') }}', selectedgroups);
        } else {
