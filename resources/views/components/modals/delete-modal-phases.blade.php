...
@props(['modalId' => 'deletePhasesModal', 'message' => 'Êtes-vous sûr de vouloir supprimer cette phase ?'])
...
<script>
    ...
    function confirmDeletephases() {
        const selectedphases = Array.from(document.querySelectorAll('input[name="selected_phases[]"]:checked')).map(checkbox => checkbox.value);
        if (selectedphases.length > 0) {
            sendPostRequest('{{ route('phases.destroySelectedPhases') }}', selectedphases);
        } else {
