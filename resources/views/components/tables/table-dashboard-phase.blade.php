...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownActionButton">
                    <li>
                        <a href="#"
                            ...
                            onclick="handleAction('create')">Créer une phase</a>
                    </li>
                </ul>
                <div class="py-1">
                    <a href="#"
                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                        onclick="handleAction('delete')">Supprimer la/les phase(s)</a>
                </div>
            </div>
        </div>
        <label for="table-search" class="sr-only">Search</label>
        <div class="relative">
            <div class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-phases-none">
            ...
            <form action="{{ route('phases.search') }}" method="GET">
                <input type="text" id="table-search-phases"
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            @include('components.modals.phase-modal', ['phase' => $phase])
            @include('components.modals.show-phase-modal', ['phase' => $phase, 'event' => $event])
            @endforeach
        </tbody>
    </table>

    <div class="mt-4">
        {{ $phases->links('components.UI.pagination') }}
    </div>
</div>
...
<script>
    function handleAction(action) {
        ...
        if (selectedphases.length > 0 || action === 'create') {
            switch (action) {
                case 'create':
                    window.location.href = "{{ route('phases.create_phasenew') }}";
...
@include('components.modals.delete-modal-phases', [
'modalId' => 'deletePhasesModal',
